				...
				GError** error)
{
	...
	{
		g_set_error(error, TCP_ERROR, TCP_ERROR_LENGTH, "Tcp length %zu < %zu", effective_length, sizeof(tcp_header));
		return NULL;
	}
...
const uint16_t
tcp_header_sport(const tcp_header_t* self)
{
	return __be16_to_cpu(((const tcp_header*)self)->source);
}	

const uint16_t
tcp_header_dport(const tcp_header_t* self)
{
